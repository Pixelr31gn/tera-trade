...
#   outputDir defaults to the parent of the repo, version defaults to "2.0.0".
...
$version = if ($args.Count -ge 2) { $args[1] } else { "2.0.0" }
...
# --- Database seed (2026-08-13, operator request: "make sure a copy of this
# database gets added to the file! so it doesn't have to re collect data!")
# -- a snapshot of THIS machine's current local Postgres (scores/bars/trades/
# equity history), taken fresh at build time, not a stale checked-in file.
# Best-effort: this build still produces a usable (empty-database) deliverable
# if Docker/the local Postgres container isn't reachable right now, since that
# was already true before this feature existed -- it just means the recipient
# starts fresh, same as every earlier .exe build. Data-only (schema comes from
# tera-trade-setup.exe's own `prisma migrate deploy`), --disable-triggers so
# FK ordering across tables doesn't need to be hand-sorted (same flag this
# project's own Neon-to-local migration already used for the identical
# reason, see docs/BUILD_HISTORY.md), custom format (-Fc) for pg_restore
# compatibility and because it's gzip-compressed automatically (a real
# ~688MB local database dumped down to ~26MB in testing).
Write-Host "`nSnapshotting local database (best-effort)..." -ForegroundColor Cyan
$seedPath = "$deliverableDir\app\db-seed.dump"
docker exec teratrade-postgres pg_dump -U teratrade -d teratrade --data-only --disable-triggers -Fc > $seedPath 2>$null
if ($LASTEXITCODE -eq 0 -and (Test-Path $seedPath) -and (Get-Item $seedPath).Length -gt 0) {
    $seedSizeMb = [Math]::Round((Get-Item $seedPath).Length / 1MB, 1)
    Write-Host "Included a $seedSizeMb MB database snapshot -- tera-trade-setup.exe will restore it into a fresh install automatically." -ForegroundColor Cyan
} else {
    if (Test-Path $seedPath) { Remove-Item -Force $seedPath }
    Write-Host "Could not snapshot the local database (Docker/Postgres not reachable right now) -- shipping without one; recipients get a fresh, empty database instead." -ForegroundColor Yellow
}

# --- Frontend (static export) ---
# No API key baked in anymore (2026-08-14) -- the dashboard now authenticates
# via the login screen (session cookie issued by POST /api/auth/login) rather
# than a key embedded in the public JS bundle, so API_KEY is free to go back
# to being a normal per-install random value generated by
# scripts\exe-setup.cjs, same as POSTGRES_PASSWORD. AUTH_PASSWORD_HASH itself
# is deliberately NOT generated here -- exe-setup.cjs prompts the recipient
# for their own dashboard password interactively; baking one into the build
# would mean every recipient of this zip shares the same login.
Write-Host "Building frontend (static export)..." -ForegroundColor Cyan
Push-Location "$root\frontend"
"NEXT_PUBLIC_API_BASE_URL=http://localhost:8000`n" | Out-File -Encoding utf8 -NoNewline ".env.local"
npm run build
...
Write-Host "  3. Double-click tera-trade-setup.exe -- restores your database snapshot and does everything else automatically, no prompts." -ForegroundColor Green
