...
async function computeStrategyComparison(): Promise<Record<"v1" | "v2" | "v3" | "v4" | "v5" | "v6", Record<TradingSession, ReturnType<typeof summarizeSessionScores>>>> {
  const versions = ["v1", "v2", "v3", "v4", "v5", "v6"] as const;
  const rows = await prisma.score.findMany({
  ...
  const results = {} as Record<"v1" | "v2" | "v3" | "v4" | "v5" | "v6", Record<TradingSession, ReturnType<typeof summarizeSessionScores>>>;
