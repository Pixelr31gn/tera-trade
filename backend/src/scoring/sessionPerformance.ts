...
// waiting for a large sample, so this is deliberately low -- matches
// consensusBandit.ts's own MIN_PER_ARM_SAMPLES_BEFORE_BANDIT=3 floor.
export const MIN_SESSION_SAMPLES_PER_VERSION = 3;
...
export async function computeSessionVersionStats(version: StrategyVersion, sessionStart: Date, at: Date): Promise<SessionVersionStats> {
  const where = { strategyVersion: version, time: { gte: sessionStart, lt: at } };
